...
sys.path.insert(0, os.path.abspath('/home/training/Aerialytic_AI/New-Graph-Extraction/line_detector_2_cls'))
...
from library.Model.object_detection.dataset import classify_line_slope , transform_keypoints ,find_intersecting_keypoints ,img_rectangle_cut , rect_scale_pad ,get_bounding_polygon
...
class Line_2_objectdetectiondataset(PhysicalCellsObjectDetectionDataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            annotations = PhysicalCellsObjectDetectionDataset.annotations_dict[label_file].copy()
            # label_path = os.path.join(self.labels_dir, label_file)
            # with open(label_path, 'rb') as f:
            #     annotations = pickle.load(f)

            aligner_data = self._load_aligner_data(annotations) if self.use_aligner else None
            ...
            if len(clipped_keypoints) > 0:
                for i in range(0, len(clipped_keypoints) - 1, 2):
                    ...
                    bbox_width = max((x_max - x_min) / self.sample_size, 0.002)
                    bbox_height = max((y_max - y_min) / self.sample_size, 0.002)
                    x_center = (x_min + x_max) / 2 / self.sample_size
                    y_center = (y_min + y_max) / 2 / self.sample_size

                    line_class = classify_line_slope(pt1, pt2)
