...
def plot_training_samples(images, batch_idx, sample_infos, paths, save_dir, ni):
    ...
    plot_images_with_keypoints(
        ...
        paths,
        save_dir,
        ni  # Passing batch index (ni) to create unique filenames
    )
...
def plot_images_with_keypoints(
    ...
    """Plot individual images with keypoints and save them in a white canvas."""
    if isinstance(images, torch.Tensor):
        images = images.cpu().float().numpy()
    if isinstance(batch_idx, torch.Tensor):
        batch_idx = batch_idx.cpu().numpy()

    bs, _, h, w = images.shape  # Get batch size, height, and width
    bs = min(bs, 16)  # Limit the number of images to 16

    if np.max(images[0]) <= 1:
        images *= 255  # Scale pixel values if needed
    ...
    for i in range(bs):
        ...
        if len(keypoints) > 0 and len(keypoints[i]) > 0:
            ...
            img_keypoints[..., 0] *= w  # Convert X to pixel
            img_keypoints[..., 1] *= h  # Convert Y to pixel
            ...
            for kpts in img_keypoints:
                for kp_idx in range(len(kpts) - 1):
                    ...
                    if v1 > 0 and v2 > 0:
                        cv2.line(canvas, (x1 + start_x, y1 + start_y), (x2 + start_x, y2 + start_y), (0, 255, 0), 2)
        ...
        img_path = save_dir / f"train_batch{ni}_img{i}.jpg"
        cv2.imwrite(str(img_path), canvas)

    return None

import os
def plot_callback(self, batch, ni):
    ...
    # Callback function to plot and save individual training samples
    plot_training_samples(
        batch['img'],
        batch['batch_idx'],
        batch['sample_infos'],  # We are passing sample_infos directly
        batch['im_file'],
        batch_dir,
        ni
    )
