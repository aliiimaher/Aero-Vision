...
sys.path.insert(0, os.path.abspath('/home/4TDrive/Yousefi/OBB/linedetector_4_cls'))
# Now import the KeypointClipper class
from callbacks.clip_keypoint import KeypointClipper
sys.path.insert(0, os.path.abspath('/home/4TDrive/Yousefi/OBB'))
...
def calculate_width(line_length):
   
        min_length, min_width = 5, 2
        max_length, max_width = 100, 20
        ...
        if line_length <= min_length:
            ...
            return min_width + (max_width - min_width) * ((line_length - min_length) / (max_length - min_length))
        
class OrientedBBoxDataset(PhysicalCellsObjectDetectionDataset):
    ...
    def create_line_bounding_box(self, pt1, pt2, img_width=1, img_height=1):
        ...
        width = calculate_width(line_length)
        
        angle = math.atan2(dy, dx)
        half_width = width / 2
        
        corners = np.array([
            [-line_length / 2, -half_width],
            [line_length / 2, -half_width],
            [line_length / 2, half_width],
            [-line_length / 2, half_width]
        ])
        ...
        norm_w = line_length / img_width
        norm_h = width / img_height

        return rotated_corners, (norm_cx, norm_cy, norm_w, norm_h, angle)
    ...
    def __getitem__(self, idx):
        ...
        if len(clipped_keypoints) > 0:
            for i in range(0, len(clipped_keypoints) - 1, 2):
                ...
                _, (cx, cy, w, h, angle) = self.create_line_bounding_box(pt1, pt2, img_width=self.sample_size, img_height=self.sample_size)
