...
class Annotator:
    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False, conf=None):
        """
        Draws a bounding box to image with color based on confidence.

        Args:
            box (tuple): The bounding box coordinates (x1, y1, x2, y2)
            label (str): Unused parameter kept for compatibility
            color (tuple, optional): The color of the rectangle (B, G, R)
            txt_color (tuple, optional): Unused parameter kept for compatibility
            rotated (bool, optional): Variable used to check if task is OBB
            conf (float, optional): The confidence score for the bounding box
        """
        if isinstance(box, torch.Tensor):
            box = box.tolist()
        
        # Map confidence to color (low confidence -> red, high confidence -> green)
        if conf is not None:
            # Create a color that goes from red (low confidence) to green (high confidence)
            red = int(255 * (1 - conf))  # Red decreases with confidence
            green = int(255 * conf)      # Green increases with confidence
            color = (red, green, 0)      # BGR format for OpenCV
        
        if self.pil:
            if rotated:
                self.draw.polygon([tuple(b) for b in box], width=1, outline=color)  # Fixed width=1
            else:
                self.draw.rectangle(box, width=1, outline=color)  # Fixed width=1
        else:  # cv2
            if rotated:
                cv2.polylines(self.im, [np.asarray(box, dtype=int)], True, color, 1)  # Fixed thickness=1
            else:
                p1, p2 = (int(box[0]), int(box[1])), (int(box[2]), int(box[3]))
                cv2.rectangle(self.im, p1, p2, color, thickness=1, lineType=cv2.LINE_AA)  # Fixed thickness=1
...
def plot_images(
    ...
    for i in range(bs):
        ...
        if len(cls) > 0:
            ...
            # Inside plot_images, where you call box_label:
            if len(bboxes):
                boxes = bboxes[idx]
                conf = confs[idx] if confs is not None else np.ones_like(cls[idx])  # If confs is None, use 1 as default confidence
                ...
                if len(boxes):
                    ...
                    # Set width or height to 1 based on which is smaller
                    if boxes.shape[-1] == 4:  # For regular boxes
                        # Compare width and height
                        widths = boxes[:, 2] - boxes[:, 0]
                        heights = boxes[:, 3] - boxes[:, 1]
                        
                        # Create masks for width > height and height > width
                        width_larger = widths > heights
                        height_larger = ~width_larger
                        
                        # For boxes where width > height, set height to 1
                        boxes[width_larger, 3] = boxes[width_larger, 1] + 1
                        
                        # For boxes where height > width, set width to 1
                        boxes[height_larger, 2] = boxes[height_larger, 0] + 1
                        
                    elif boxes.shape[-1] == 5:  # For rotated boxes
                        # For rotated boxes, adapt this logic based on format
                        # Assuming format is [x, y, w, h, angle]
                        width_larger = boxes[:, 2] > boxes[:, 3]
                        height_larger = ~width_larger
                        
                        # Set width to 1 where height is larger
                        boxes[height_larger, 2] = 1
                        
                        # Set height to 1 where width is larger
                        boxes[width_larger, 3] = 1
                ...
                for j, box in enumerate(boxes.astype(np.int64).tolist()):
                    ...
                    # If confidence is available, use it, otherwise skip or use default color
                    if labels or conf[j] > conf_thres:
                        label = f"{c}" if labels else f"{c} {conf[j]:.1f}"
                        annotator.box_label(box, label, color=color, rotated=is_obb, conf=conf[j])  # Pass confidence here
